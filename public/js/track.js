...
(function () {
  ...
  function waLink() {
    var phone = CONFIG.WHATSAPP_PHONE.replace(/[^0-9]/g, '');
    return 'https://wa.me/' + phone;
  }

  function waError(html) {
    var errorEl = document.getElementById('track-error');
    errorEl.innerHTML = html + ' Please <a href="' + waLink() + '" target="_blank" rel="noopener" style="color:var(--primary-dark);font-weight:700;text-decoration:underline;">contact us on WhatsApp</a>.';
  ...
  function trackOrder() {
    ...
    if (!CONFIG.SUPABASE_URL || !CONFIG.SUPABASE_KEY || CONFIG.SUPABASE_URL.indexOf('__VITE_') === 0) {
      waError('Order tracking is not configured.');
    ...
    .catch(function () {
      waError('Something went wrong. Please try again or');
      resultEl.innerHTML = '';
    });
  }
